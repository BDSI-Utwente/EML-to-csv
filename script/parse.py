...
def extract_elected_candidates(xml_file_path):
    ...
    for selection in root.findall(".//Selection", namespaces):
        ...
        if candidate is not None:
            ...
            candidate_short_code = candidate.find(
                "CandidateIdentifier", namespaces
            ).get("ShortCode")
            ...
            results.append(
                {
                    ...
                    "candidate_short_code": candidate_short_code,
...
def extract_region_definitions(xml_file_path):
    namespaces = {
        "": "urn:oasis:names:tc:evs:schema:eml",
        "ds": "http://www.w3.org/2000/09/xmldsig#",
        "kr": "http://www.kiesraad.nl/extensions",
    ...
    for region in root.findall(".//kr:Region", namespaces):
        region_id = region.get("RegionNumber")
        region_type = region.get("RegionCategory")
        region_name_element = region.find("kr:RegionName", namespaces)
        region_name = (
            region_name_element.text if region_name_element is not None else ""
        )

        superior_region_id = region.get("SuperiorRegionNumber")
        superior_region_type = region.get("SuperiorRegionCategory")
        ...
        regions.append(
            {
                ...
                "superior_region_type": superior_region_type,
            }
        )
...
def parse_files(source_dir: Path, target_dir: Path, per_unit=False):
    ...
    for index, path in enumerate(paths):
        print(f"\r[{index + 1}/{len(paths)}]: {path}\t\t", end=None)
        total_votes.extend(extract_votes(path))
    ...
    # election definition
    paths = list(source_dir.glob("**/Verkiezingsdefinitie_*.xml"))
    print("Processing election region definitions...")
    election_definitions = []
    for index, path in enumerate(paths):
        print(f"\r[{index + 1}/{len(paths)}]: {path}\t\t", end=None)
        election_definitions.extend(extract_region_definitions(path))
    print("Writing election region definitions to csv...")
    df = pd.DataFrame(election_definitions)
    path = target_dir / "region_definitions.csv"
    df.to_csv(str(path), index=False, encoding="utf-8")
    print("Done!")

    if per_unit:
        print("Processing per-reporting-unit vote counts...")
        paths = list(source_dir.glob("**/Telling_*.xml"))
        per_council_votes = []
        for index, path in enumerate(paths):
            print(f"\r[{index + 1}/{len(paths)}]: {path}\t\t", end=None)
            per_council_votes.extend(extract_votes(path))
        print("Writing per-reporting-unit votes to csv...")
        df = pd.DataFrame(per_council_votes)
        path = target_dir / "votes_per_council.csv"
        df.to_csv(str(path), index=False, encoding="utf-8")
        print("Done!")
...
if __name__ == "__main__":
    ...
    parse_files(SOURCE, TARGET, args["--per_unit"])
